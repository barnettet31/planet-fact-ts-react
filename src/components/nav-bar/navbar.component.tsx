...
function PlanetLink ({name, path, color, linkClickedFunction}:IPlanetPathData){
    return ( <Link onClick={linkClickedFunction} className="w-full" to={path}><li className="w-full "><div className="flex items-center justify-between w-full py-5 md:pb-7 md:py-0 lg:pb-0"><div className="flex items-center gap-6"><div className={`w-5 h-5 rounded-full ${color} md:hidden`}></div><p className="font-body font-bold font-[15px] leading-[25px] tracking-[1.36px]">{name.toUpperCase()}</p></div><img className="w-1 h-2 md:hidden" alt='' src={`${process.env.PUBLIC_URL}/images/icon-chevron.svg`}/></div></li></Link>)
}
export function NavBar (){
    ...
    const handleNavBurgerClick = ()=> setNavShown(!navShown);
        ...
        <div className="relative flex md:flex-col md:gap-10 lg:flex-row items-center justify-between px-6 py-4 border-b border-b-white/[.2]">
            ...
            <nav className={`fixed  h-screen w-screen  bg-[#070724] md:bg-transparent md:static md:w-full md:h-auto ease-in-out duration-200 top-[69px]  px-6 py-11 md:py-2 left-0 origin-top ${navShown? 'scale-y-100 opacity-100 md:opacity-100 md:scale-y-100':'scale-y-0 opacity-70 '} `}>
