...
function SelectedList({ selectedItem, updateSelected, planet }: ISelectedListProps) {
  const planets = [
    {
      name: "Mercury",
  ...
  return (
    <ul className="flex justify-between w-full py-5 border-b-white/[0.2] border-b px-6">
      <li
        onClick={() => updateSelected("overview")}
        className={`relative after:absolute ${
          selectedItem === "overview" ? "after:scale-x-100" : "after:scale-x-0"
        } after:ease-in-out after:origin-left after:duration-200 after:content-[''] after:h-[4px] after:w-full  after:-bottom-[80%] ${planets.find(element=>element.name === planet)?.color}`}
      >
        <BodyText
          additionalStyles={
      ...
      <li
        ...
        }  after:content-[''] after:h-[4px] after:duration-200 after:ease-in-out after:origin-left after:w-full after:-bottom-[80%]  ${planets.find(element=>element.name === planet)?.color}`}
      >
        <BodyText
          additionalStyles={
